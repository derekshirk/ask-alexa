...
class ACA_ACF_Column extends AC_Column_Meta
	...
	public function register_settings() {
		$setting = 'Setting_Field_' . ucfirst( $this->get_list_screen()->get_group() );

		// Default version
		$class = ac_addon_acf()->get_class_prefix() . $setting;

		// Free version specific
		if ( ac_addon_acf()->is_acf_free() ) {
			$free_class = ac_addon_acf()->get_class_prefix() . 'Free_' . $setting;
	...
	public function get_field_by_type( $field_type ) {
		$type = '_' . AC_Autoloader::string_to_classname( $field_type );

		// Default version
		$class = ac_addon_acf()->get_class_prefix() . 'Field';
		...
		if ( ac_addon_acf()->is_acf_free() ) {
			...
			$free_class = ac_addon_acf()->get_class_prefix() . 'Free_Field';

			if ( class_exists( $free_class . $type ) ) {
				$class = $free_class . $type;
			}
		}
