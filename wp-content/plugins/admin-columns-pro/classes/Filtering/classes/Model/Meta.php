...
class ACP_Filtering_Model_Meta extends ACP_Filtering_Model {
	 ...
	 * @return array
	 */
	protected function get_filtering_vars_date( $vars, array $args = array() ) {
		$defaults = array(
			'date_format'   => 'Y-m-d',
			'filter_format' => 'daily',
		);
		...
		if ( $this->is_ranged() ) {
			...
			if ( $value['min'] ) {
				$args['min'] = date( $args['date_format'], strtotime( $value['min'] ) );
			}

			if ( $value['max'] ) {
				$args['max'] = date( $args['date_format'], strtotime( $value['max'] ) );
			}
		...
		switch ( $args['filter_format'] ) {
			...
			case 'yearly' :
				if ( $date = $this->get_date_time_object( $value . '0101' ) ) {
					...
					$args['min'] = $date->format( $args['date_format'] );
					$args['max'] = $date->modify( '+1 year' )->modify( '-1 day' )->format( $args['date_format'] );
			...
			case 'monthly' :
				if ( $date = $this->get_date_time_object( $value . '01' ) ) {
					...
					$args['min'] = $date->format( $args['date_format'] );
					$args['max'] = $date->modify( '+1 month' )->format( $args['date_format'] );
			...
			case 'daily' :
				if ( $date = $this->get_date_time_object( $value ) ) {
					...
					$args['min'] = $date->format( $args['date_format'] );
					$args['max'] = $date->modify( '+1 day' )->format( $args['date_format'] );
