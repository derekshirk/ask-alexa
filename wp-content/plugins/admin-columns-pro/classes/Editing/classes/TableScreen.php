...
class ACP_Editing_TableScreen {
	...
	public function scripts( $list_screen ) {
		...
		// Main
		wp_register_script( 'acp-editing-table', $plugin_url . 'assets/js/table' . $minified . '.js', array( 'jquery', 'acp-editing-bootstrap-editable', 'acp-editing-select2' ), $version );
	...
	private function get_column_items( $columns ) {
		...
		foreach ( $columns as $column ) {
			...
			if ( ! $rows ) {
				...
				wp_cache_add( $column->get_list_screen()->get_storage_key(), $editing->get_strategy()->get_rows(), 'editable-rows', 10 );
			}
