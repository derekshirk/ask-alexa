 ...
 *
 */
class ACP_ExportImport extends AC_Admin_Page {
	...
	function __construct() {
		...
		AC()->admin()->get_pages()->register_page( $this );
		...
		foreach ( AC()->get_list_screens() as $list_screen ) {
			...
			if ( in_array( $list_screen->get_key(), $types ) ) {
				foreach ( ACP()->layouts( $list_screen )->get_layouts() as $layout ) {
					if ( $columns = $list_screen->set_layout_id( $layout->get_id() )->get_settings() ) {
						$data[ $list_screen->get_key() ][] = array(
							'columns' => $columns,
			...
			else if ( $layouts = ACP()->layouts( $list_screen )->get_layouts() ) {
	...
	private function get_export_multiselect_options() {
		...
		foreach ( AC()->get_list_screens() as $list_screen ) {
			...
			if ( $export_single_layouts ) {
				...
				// Layouts
				if ( $layouts = ACP()->layouts( $list_screen )->get_layouts() ) {
					foreach ( $layouts as $layout ) {
			...
			<?php if ( $this->php_export_string ) : ?>
                <tr>
                    <th scope="row">
                        ...
                        <p>
                            <a href="#" class="ac-pointer" rel="pac-php-export-instructions-html" data-pos="right"><?php _e( 'Instructions', 'codepress-admin-columns' ); ?></a>
                        </p>
                        <div id="pac-php-export-instructions-html" style="display:none;">
                            <h2><?php _e( 'Using the PHP export', 'codepress-admin-columns' ); ?></h2>
                            <ol>
